...
{
  int i, checksum=0; /*First init to Zero*/
  if(p == NULL)  return checksum;
...
void printLog(int AorB, char *msg, struct pkt *p, struct msg *m)
{
    ...
    {
        if(p != NULL) {
            printf("[Sender] %s. Window[%d,%d) Packet[seq=%d,ack=%d,check=%d,data=%s]\n",  msg,
            base, nextseq, p->seqnum, p->acknum, p->checksum, p->payload);
        } 
        else if(m != NULL) 
        {
          printf("[Sender] %s. Window[%d,%d) Message[data=%c..]\n",  msg, base, nextseq, m->data[0]);
        } 
        else {
          printf("[Sender] %s.Window[%d,%d)\n", msg, base, nextseq);
        }
    }
    else
    {
      if(p != NULL) {
          printf("[Receiver] %s. Expected[%d] Packet[seq=%d,ack=%d,check=%d,data=%s]\n",  msg,
          B_seqnum, p->seqnum, p->acknum, p->checksum, p->payload);
      } else if(m != NULL) {
          printf("[Receiver] %s. Expected[%d] Message[data=%s]\n",  msg, B_seqnum, m->data);
      } else {
          printf("[Receiver] %s. Expected[%d]\n", msg, B_seqnum);
      }
    }
}
...
struct pkt *get_free_packet()
{
  ...
  if(nextseq - base >= N)
  {
    printLog(A, "Alloc packet failed. The window is full already", NULL, NULL);
    return NULL;
  }
...
struct pkt *get_packet(int seqnum)
{
  ...
  {
    printLog(A, "Seqnum is not within the window", NULL, NULL);
    return NULL;
  }
...
setPecket(packet)
  struct pkt *packet;
{
  packet->seqnum = nextseq; ////设置下一个的nextseq
  // we are send package, do not need to set acknum 
  packet->acknum = DEFAULT_ACK;
  ////发送数据包，acknum设置为默认值，不需要特殊指定
  int cksum=getChecksum(packet);
  packet->checksum = cksum; //设置校验和
}
  ...
  struct msg message;
{
  ...
  //第二步，判断nextseq顺序关系
  ////然后弹出msg
  if(nextseq >= base + N)  ////if窗口已经满了
  {
    printLog(A, "Window is full already, save message to extra buffer", NULL, &message);
    return;
  }
  ...
  if(p == NULL)
  {
    printLog(A, "BUG! The window is full already", NULL, &message);
    return;
  }
  ++A_from_layer5;
  printLog(A, "Receive an message from layer5", NULL, &message);
  
  //将payload作为data，赋给message作为数据包发出去
  copyStr20(n->message.data, p->payload);
  ...
  printLog(A, "Send packet to layer3", p, &message);
  //printf("[A_output] End\n");
  return 0;
}
...
A_input(packet)
  struct pkt packet;
{
  //printf("[A_input] Begin:\n");
  printLog(A, "Receive ACK packet from layer3", &packet, NULL);

  /* check checksum, if corrupted, do nothing */
  if(packet.checksum != getChecksum(&packet))
  {
    printLog(A, "ACK packet is corrupted", &packet, NULL);
    return;
  }
  ...
  {
    printLog(A, "Receive duplicate ACK", &packet, NULL);
    return;
  }
  else if(packet.acknum >= nextseq)
  {
    printLog(A, "BUG: receive ACK of future packets", &packet, NULL);
    return;
  }
  ...
  printLog(A, "ACK packet process successfully accomplished!!", &packet, NULL);
  //printf("[A_input] End\n"); 
}
...
{
  int i;
  printLog(A, "[A_timerinterrupt] Time interrupt occurs", NULL, NULL);
  ...
  for(i = base; i < nextseq; ++i)
  {
    ...
    printLog(A, "Timeout! Send out the package again", p, NULL);
  }
...
B_input(packet)
  struct pkt packet;
{
    printLog(B, "Receive a packet from layer3", &packet, NULL);
    ++B_from_layer3;

  // 检查cksum，如果损坏，则丢弃该包
  if(packet.checksum != getChecksum(&packet))
  {
        printLog(B, "Packet is corrupted", &packet, NULL);
        return;
  }
  ...
  if(B_seqnum - 1 >= 0)
  {
      ...
      tolayer3(B, packet);
      printLog(B, "Send ACK packet to layer3", &packet, NULL);
  }
  //printf("[B_input(packet)] End\n");
}

/* called when B's timer goes off */
B_timerinterrupt()
{
    printf("[B_timerinterruput] B have no timer. Ignore.");
}
